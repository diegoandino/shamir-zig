...
const SERVER_PRIME = ;
    ...
    if (req.body()) |body| {
        ...
        if (try req.json(InitRequest)) |parsed| {
            ...
            var SSSS = scheme.ShamirsSecretSharingScheme.init(
                ...
                parsed.threshold,
                parsed.total_shares,
                try Managed.initSet(allocator, SERVER_PRIME)
            );
